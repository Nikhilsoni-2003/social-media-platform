       ...
       <div className='w-full h-[70px] flex items-center element-gap glass rounded-xl container-padding'>
                  ...
                  <div className='w-full flex items-center justify-center'>
                    <div className='w-full max-w-[600px] glass rounded-xl flex items-center element-gap container-padding'>
                      ...
                      <input type="text" placeholder='Search users...' className='flex-1 outline-0 bg-transparent text-primary placeholder-muted text-base' onChange={(e)=>setInput(e.target.value)} value={input}/>
                    </div>
                  </div>
                  
   {input && searchData?.map((user, index)=>(
<div key={index} className='w-full max-w-[600px] glass rounded-xl flex items-center element-gap container-padding cursor-pointer hover-lift transition-all duration-300' onClick={()=>navigate(`/profile/${user.userName}`)}>
